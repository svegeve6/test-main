    ...
    >
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-3 lg:space-y-0">
        <div className="flex items-center space-x-3">
          <div className={`
            p-2 rounded-xl 
            ${enabled ? 'bg-white/[0.08] backdrop-blur-sm' : 'bg-white/[0.03]'}
...
const InputField = ({ icon: Icon, label, value, onChange, type = 'text' }) => {
  ...
  return (
    <div 
      className="px-4 py-3.5 transition-all duration-300 hover:bg-white/[0.03]"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className="flex items-center space-x-3">
        <div className={`
          p-2 rounded-xl bg-white/[0.03] backdrop-blur-sm
