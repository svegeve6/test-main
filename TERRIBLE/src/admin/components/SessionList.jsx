...
import { Ban, ExternalLink, Monitor, MapPin, Trash2, Edit2, Check, X } from 'lucide-react';
...
const SessionAlias = ({ sessionId, isDesktop = false }) => {
  ...
  const [alias, setAlias] = useState(() => {
    return localStorage.getItem(`session-alias-${sessionId}`) || sessionId.slice(0, 8);
  });
  ...
  const handleSave = () => {
    ...
    setAlias(newAlias);
    localStorage.setItem(`session-alias-${sessionId}`, newAlias);
    setIsEditing(false);
  };
