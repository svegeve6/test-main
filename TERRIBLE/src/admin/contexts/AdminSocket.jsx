...
function AdminSocketProvider({ children }) {
  ...
  const [callers, setCallers] = useState(() => {
    // Load callers from localStorage
    const stored = localStorage.getItem('callers');
  ...
  const addCaller = (callerData) => {
    ...
    const updated = [...callers, newCaller];
    setCallers(updated);
    localStorage.setItem('callers', JSON.stringify(updated));
  };

  const updateCaller = (id, updatedData) => {
    const updated = callers.map(c =>
